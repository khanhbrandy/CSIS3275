...
import com.example.PersonalFinanceApp.model.GoalRepository;
...
public class GoalController {
	...
	public ResponseEntity<List<Goal>> getAllGoals(@PathVariable("user_id") long user_id) {
		try {
			...
			// To-do
			allGoals = goalRepository.getbyUser_id(user_id);
			if (allGoals.isEmpty()) {
...
try {
	Optional<Goal> goalData = goalRepository.findByIdAndUser_Id(goal_id , user_id);
...
public ResponseEntity<HttpStatus> deleteGoal(@PathVariable("user_id") long user_id,@PathVariable("goal_id") long goal_id) {
	try {
		goalRepository.deleteByIdAndUser_id( goal_id, user_id);
		return new ResponseEntity<>(HttpStatus.NO_CONTENT);
	} catch (Exception e) {
		return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
	}
